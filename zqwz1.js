...
let zqwzbody1= $.isNode() ? (process.env.zqwzbody1 ? process.env.zqwzbody1 : "") : ($.getdata('zqwzbody1') ? $.getdata('zqwzbody1') : "")
let zqwzbody1Arr = []
let zqwzbody1s = ""

let zq_timebody1= $.isNode() ? (process.env.zq_timebody1 ? process.env.zq_timebody1 : "") : ($.getdata('zq_timebody1') ? $.getdata('zq_timebody1') : "")
let zq_timebody1Arr = []
let zq_timebody1s = ""
let zqwznum
...
if (zq_timebody1) {
    if (zq_timebody1.indexOf("&") == -1) {
        zq_timebody1Arr.push(zq_timebody1)
    } else if (zq_timebody1.indexOf("&") > -1) {
        zq_timebody1s = zq_timebody1.split("&")
    } else if (process.env.zq_timebody1 && process.env.zq_timebody1.indexOf('&') > -1) {
        zq_timebody1Arr = process.env.zq_timebody1.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()) {
    var fs = require("fs");
    zq_timebody1 = fs.readFileSync("zq_timebody1.txt", "utf8");
...
Object.keys(zq_timebody1s).forEach((item) => {
    if (zq_timebody1s[item] && !zq_timebody1s[item].startsWith("#")) {
        zq_timebody1Arr.push(zq_timebody1s[item])
    }
})

if (zqwzbody1) {
    if (zqwzbody1.indexOf("&") == -1) {
        zqwzbody1Arr.push(zqwzbody1)
    } else if (zqwzbody1.indexOf("&") > -1) {
        zqwzbody1s = zqwzbody1.split("&")
    } else if (process.env.zqwzbody1 && process.env.zqwzbody1.indexOf('&') > -1) {
        zqwzbody1Arr = process.env.zqwzbody1.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()) {
    var fs = require("fs");
    zqwzbody1 = fs.readFileSync("zqwzbody1.txt", "utf8");
    if (zqwzbody1 !== `undefined`) {
        zqwzbody1s = zqwzbody1.split("\n");
    } else {
...
Object.keys(zqwzbody1s).forEach((item) => {
    if (zqwzbody1s[item] && !zqwzbody1s[item].startsWith("#")) {
        zqwzbody1Arr.push(zqwzbody1s[item])
    }
})
...
!(async () => {
    if (typeof $request !== "undefined") {
     ...
     $.done()
        ...
        console.log(`共${zqwzbody1Arr.length}个阅读body`)
        index1 = indexLast * 1
        for (let k =  index1 ? index1 : 0; k < zqwzbody1Arr.length; k++) {
            // $.message = ""
            zqwzbody11 = zqwzbody1Arr[k];
            // console.log(`${zqwzbody11}`)
            console.log(`--------第 ${k + 1} 次阅读任务执行中--------\n`)
            await wzjl()
            zqwznum = k+2
            $.setdata(zqwznum, 'zqbody_index');
            await $.wait(60000);
            for (let k = 0; k < zq_timebody1Arr.length; k++) {
                zq_timebody11 = zq_timebody1Arr[k];
                await timejl()
            }
            console.log("\n\n")
        }
...
function getzqwzbody1() {
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/article\/info.json/)||$request.url.match(/\/kandian.wkandian.com\/v5\/article\/detail.json/)) {
        ...
        if (zqwzbody1) {
            if (zqwzbody1.indexOf(bodyVal) > -1) {
                $.log("此阅读请求已存在，本次跳过")
            } else if (zqwzbody1.indexOf(bodyVal) == -1) {
                zqwzbody1s = zqwzbody1 + "&" + bodyVal;
                $.setdata(zqwzbody1s, 'zqwzbody1');
                $.log(`${$.name}获取阅读: 成功, zqwzbody1s: ${bodyVal}`);
                bodys = zqwzbody1s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个阅读请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqwzbody1');
            $.log(`${$.name}获取阅读: 成功, zqwzbody1s: ${bodyVal}`);
    ...
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/user\/stay.json/)) {
        ...
        if (zq_timebody1) {
            if (zq_timebody1.indexOf(bodyVal) > -1) {
                ...
                zq_timebody1s = zq_timebody1 + "&" + bodyVal;
                $.setdata(zq_timebody1s,'zq_timebody1');
                $.log(`${$.name}获取阅读: 成功, zq_timebody1s: ${bodyVal}`);
                bodys = zq_timebody1s.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个阅读请求: 成功🎉", ``)
            }
        } else {
