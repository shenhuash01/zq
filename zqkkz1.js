...
let zqlookStartbody1= $.isNode() ? (process.env.zqlookStartbody1 ? process.env.zqlookStartbody1 : "") : ($.getdata('zqlookStartbody1') ? $.getdata('zqlookStartbody1') : "")
let zqlookStartbody1Arr = []
let zqlookStartbody1s = ""
...
if (zqlookStartbody1) {
    if (zqlookStartbody1.indexOf("&") == -1) {
        zqlookStartbody1Arr.push(zqlookStartbody1)
    } else if (zqlookStartbody1.indexOf("&") > -1) {
        zqlookStartbody1s = zqlookStartbody1.split("&")
    } else if (process.env.zqlookStartbody1 && process.env.zqlookStartbody1.indexOf('&') > -1) {
        zqlookStartbody1Arr = process.env.zqlookStartbody1.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zqlookStartbody1 = fs.readFileSync("zqlookStartbody1.txt", "utf8");
    if (zqlookStartbody1 !== `undefined`) {
        zqlookStartbody1s = zqlookStartbody1.split("\n");
    } else {
...
Object.keys(zqlookStartbody1s).forEach((item) => {
    if (zqlookStartbody1s[item] && !zqlookStartbody1s[item].startsWith("#")) {
        zqlookStartbody1Arr.push(zqlookStartbody1s[item])
    }
})

!(async () => {
     if (typeof $request !== "undefined") {
     await getzqlookStartbody1()
     $.done()
 }else{
    console.log(`共${zqlookStartbody1Arr.length}个看看赚body`)
	        for (let k = 0; k < zqlookStartbody1Arr.length; k++) {

                zqlookStartbody11 = zqlookStartbody1Arr[k];
...
//获取看看赚激活body
async function getzqlookStartbody1() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zqlookStartbody1) {
            if (zqlookStartbody1.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zqlookStartbody1.indexOf(bodyVal) == -1) {
                zqlookStartbody1s = zqlookStartbody1 + "&" + bodyVal;
                $.setdata(zqlookStartbody1s, 'zqlookStartbody1');
                $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody1s: ${bodyVal}`);
                bodys = zqlookStartbody1s.split("&")
                 $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqlookStartbody1');
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbody1s: ${bodyVal}`);
...
function lookStart(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            body : zqlookStartbody11,}//xsgbody,}
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody1').replace(zqlookStartbody11 + "&", "");
                    $.setdata(smbody, 'zqlookStartbody1');
